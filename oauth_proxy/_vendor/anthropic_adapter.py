...
Translates between an OpenAI-style message format and Anthropic's Messages
...
# Caller effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Older configurations persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved locally)
    ...
    # 1. Locally-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for older configurations.
...
# ── Local PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.oauth-proxy/.anthropic_oauth.json (a local file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_PROXY_OAUTH_FILE = _app_home() / ".anthropic_oauth.json"
...
def run_oauth_login_pkce() -> Optional[Dict[str, Any]]:
    """Run a local OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize this proxy with your Claude Pro/Max subscription.")
    print()
...
def read_local_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read locally-managed OAuth credentials from ~/.oauth-proxy/.anthropic_oauth.json."""
    if _PROXY_OAUTH_FILE.exists():
        try:
            data = json.loads(_PROXY_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read local OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See issue 13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See issues 13848 (Kimi) and
    16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        # 2. Prefix tool names with mcp_ (Claude Code convention)
        if anthropic_tools:
            for tool in anthropic_tools:
                if "name" in tool:
                    tool["name"] = _MCP_TOOL_PREFIX + tool["name"]

        # 3. Prefix tool names in message history (tool_use and tool_result blocks)
    ...
    # silently hides reasoning text that callers may surface in their CLIs. We
